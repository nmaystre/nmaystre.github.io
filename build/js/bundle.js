...
var statsGraph = document.querySelector('.stats__graph');
if (statsGraph) {
    plotters.forEach(function (data) {
        var canvas = document.querySelector('#myChart' + data.index);
        var chartWrp = canvas.getContext('2d');
        var plotOptions = {
        ...
        data.data.values.forEach(function (category) {
            ...
            // @ts-ignore
            for (var _i = 0, _a = Object.values(category); _i < _a.length; _i++) {
                var value = _a[_i];
        ...
        var myChart = new Chart(chartWrp, {
            type: 'line',
            data: plotOptions,
            options: {
                responsive: true,
